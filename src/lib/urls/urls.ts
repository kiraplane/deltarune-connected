...
export const CANONICAL_BASE_URL = 'https://www.deltarune-connected.wiki';
...
function getProductionBaseUrl(url?: string) {
  ...
  try {
    ...
    if (
      parsedUrl.hostname === 'deltarune-connected.wiki' ||
      parsedUrl.hostname === 'www.deltarune-connected.wiki'
    ) {
...
export function getBaseUrl(): string {
  ...
  return getProductionBaseUrl(configuredBaseUrl) ?? getCanonicalBaseUrl();
}
...
export function getCanonicalImageUrl(image: string): string {
  ...
  return `${getCanonicalBaseUrl()}${image.startsWith('/') ? image : `/${image}`}`;
}
